...
const fieldsByCondition: Record<Condition, string[]> = {
  [Condition.Fat]: ['genre', 'bodyMass', 'height', 'age', 'levelOfActivity'],
  [Condition.Eutrophic]: ['genre', 'bodyMass', 'height', 'age', 'levelOfActivity'],
  [Condition.Athletic]: ['bodyMass', 'levelOfActivity'],
  [Condition.FatFreeMass]: ['fatFreeMass'],
}

export default function TMRForm({ onSubmit }: TMRFormProps) {
  const { isCalculating, setIsCalculating, setResult } = useAppStore()
  const [delayToChange, setDelayToChange] = useState(false)
  const {
    control,
    setFocus,
    handleSubmit,
  ...
  const fields = useCallback(
    (fieldName: string, duration: number) => {
      switch (fieldName) {
        case 'genre':
          return (
            <Controller
  ...
  const resetFields = useCallback(() => {
    ...
    resetField('genre')
    setTimeout(() => setDelayToChange(false))
  }, [resetField])
  ...
  return (
    <YStack rowGap="$2">
      ...
      />
      {condition && !delayToChange ? (
        fieldsByCondition[condition].map((field, index) => fields(field, 150 * index + 1))
      ) : (
        <></>
      )}
      <AnimatedStack entering={FadeInUp.delay(50).duration(150).springify()}>
        <GradientButton
          title="Calcular"
